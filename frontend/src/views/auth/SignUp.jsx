...
function SignUp() {
  ...
  const handleSubmit = (e) => {
    ...
    if (cleanUsername.length < 3 || emailError || password.length < 6) return
  ...
  return (
    <Flex direction='column' justify='center' mx='lg' style={{ height: '70dvh' }}>
      ...
      <form onSubmit={handleSubmit}>
        <Stack>
          ...
          <Button
            type='submit'
            loading={isPending}
            disabled={!email || !password || emailError || email.length < 3 || password.length < 6}
            fullWidth
          >
            Registrati
          </Button>
          ...
          {error && error?.code !== 'user_already_exists' && (
            <Alert icon={<IconAlertCircle size={16} />} color='error'>
